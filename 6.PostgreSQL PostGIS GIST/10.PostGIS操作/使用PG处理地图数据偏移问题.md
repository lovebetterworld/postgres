- [使用PG处理地图数据偏移问题_GAMESLI-GIS的博客-CSDN博客](https://blog.csdn.net/qq_22849251/article/details/118331609)

# 问题背景

进行[数据可视化](https://so.csdn.net/so/search?q=数据可视化&spm=1001.2101.3001.7020)，客户给的数据是火星偏移经纬度数据，和地图底图（wgs84）坐标系不同，因此叠加上去有偏移，因此寻找工具对数据源进行纠偏，找到一个PG工具函数可以进行纠偏，地址：https://github.com/FreeGIS/Postgis_Coordinate_Transform/blob/master/FreeGIS_Coordinate_Transform.sql

## 1.创建数据库，并新增posgis扩展

## 2.运行如下[sql语句](https://so.csdn.net/so/search?q=sql语句&spm=1001.2101.3001.7020)，新增工具函数FreeGIS_Coordinate_Transform
...
## 3 使用PostGIS Shapefile Import/Export Manager数据工具导入shp数据源到PG中

切记SRID需要手动填入4326坐标系编号

![在这里插入图片描述](https://img-blog.csdnimg.cn/20210629115703127.png)
...
```sql
select FreeGIS_Coordinate_Transform('public','表名称','GCJ2WGS');
```